/* This program identifies the rank of each analogical API using the ground truth 
 * for Indexing approach */
package FYPLucene;
...
public class FindRanks {
	 ...
	 static String filePath = "/Using API descriptions only\\Results";
    ...
    public static void FindRanks(ArrayList<String> AL, String fileName) throws IOException{
    	...
    	BufferedReader br = new BufferedReader(new FileReader(filePath + "\\" + fileName.split("\\.txt")[0] + ".txt"));
    		...
    		if (lineArray.length > 1){
		    	...
		    	for (String matchAPI: AL){
			    	for (int i = 0; i < sortedResult.size(); i++){
			    		...
			    		if (lineArray.length == 2 && mapRanks != null){
			    		...
			    		if (mapRanks != null && i == sortedResult.size() - 1){
			    			if (mapRanks.get(targetResult) != null){
			    				...
			    				mapResults.put(sourceResult + "\t" + targetResult + "\t200", "");
				    			sourceResult = "";
				    			targetResult = "";
				    			mapRanks = null;
			    			}
