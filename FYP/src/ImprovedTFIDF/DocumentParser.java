/* This program builds vocabulary to compute modified TF-IDF for API description */
...
public class DocumentParser {
	...
	HashMap<String, Integer> docFreqOfWords = new HashMap<String, Integer>();	// Number of documents the term appear in
	...
	static String outputFilePath = "/API Name and Descriptions vectors (Improved TFIDF)";
            ...
            while ((s = in.readLine()) != null) {
            	if (s.trim().split("###").length > 1 && !s.trim().split("###")[1].trim().equals("") &&  !s.trim().split("###")[1].trim().equals("\u00A0")) {
	                ...
	                for (String term : tokenizedTerms) {
	                	if (docFreqOfWords.containsKey(term.toLowerCase()) && !appearBeforeList.contains(term.toLowerCase())){
	                		docFreqOfWords.put(term.toLowerCase(), docFreqOfWords.get(term.toLowerCase()) + 1);
	                		appearBeforeList.add(term.toLowerCase());
	                	}
	                	else if (!appearBeforeList.contains(term.toLowerCase())){
	                		docFreqOfWords.put(term.toLowerCase(), 1);
	                		appearBeforeList.add(term.toLowerCase());
	                	}
    ...
    /**
     * Method to create termVector according to its TF-IDF score.
     * @throws IOException 
     */
	    		...
	    		while ((line = in.readLine()) != null) {
	    			if (line.trim().split("###").length > 1 && !line.trim().split("###")[1].trim().equals("") &&  !line.trim().split("###")[1].trim().equals("\u00A0")) {
	            		...
	            		for (String eachTerm: tokenizedTerms){
	            			...
	            			idf = new TfIdf().idfCalculator(docFreqOfWords.get(eachTerm.toLowerCase()), NumOfDoc);
    ...
    /**
     * Check if the vector of a library has already been generated
     */
