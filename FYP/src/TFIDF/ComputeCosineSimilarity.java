...
public class ComputeCosineSimilarity {
	...
	static String inputFilePath = "/API Name and Descriptions vectors (TFIDF)";
	static String outputFilePath = "/API descriptions tf-idf results";
	...
	public static void computeCSBetweenLib(File[] files, String similarLib) throws IOException {
	   for (File file : files) {
	        if (file.isDirectory()) {
	            ...
	            computeCSBetweenLib(file.listFiles(), similarLib); // Calls same method again.
	        } else {
	...
	private static void readFile(String similarLib) throws IOException {
		...
		FileWriter fwStream = new FileWriter(inputFilePath + "\\" + similarLib.split("###")[0] + " vs " + similarLib.split("###")[1] + ".csv", true);
